...
#
# Output: intellij/bin/<os>-<arch>/schemalock[.exe]
#
...
APP_DIR="$(cd "${INTELLIJ_DIR}/../app" && pwd)"
BIN_DIR="${INTELLIJ_DIR}/bin"
...
if [ "${BUILD_MODE}" = "dev" ]; then
  if [ -f "${INTELLIJ_DIR}/.app-version" ]; then
    PIN_BASE=$(tr -d '[:space:]' < "${INTELLIJ_DIR}/.app-version" | sed 's/^v//')
  else
    PIN_BASE="0.0.0"
  fi
  ...
  if [ ! -f "${INTELLIJ_DIR}/.app-version" ]; then
    echo "ERROR: intellij/.app-version missing — required for release builds" >&2
    exit 1
  fi
  PIN_TAG=$(tr -d '[:space:]' < "${INTELLIJ_DIR}/.app-version")
